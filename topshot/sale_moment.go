...
func GetSaleMomentFromOwnerAtBlock(flowClient *client.Client, blockHeight uint64, ownerAddress flow.Address, momentFlowID uint64) (*SaleMoment, error) {
	/*
	- import from TopShot from a particular block in the chain
	- import Market from a particular block MomentPurchasedEventin the chain
	- the chain is maintained by the network of computers performing verification of new blocks as well as minting and etc...?
	- bandwidth would be contingent upon the computational power of the network reliant upon the machines (metal) and the software...?
	- we denote the script to be ran on the Flow network in the following
	- appears to be possible a creation of own SaleMoment structure
	 */
	getSaleMomentScript := `
...
`
	// execute script on the flow network, making an RPC
